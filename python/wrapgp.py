...
class WrappedGaussianProcess(MultiOutputMixin, RegressorMixin, BaseEstimator):
    ...
    def __init__(self, space, prior):
        ...
        self.euclidean_gpr = GaussianProcessRegressor(
            kernel = ConstantKernel(1.0, constant_value_bounds="fixed") * RBF(1.0, length_scale_bounds="fixed") + WhiteKernel(noise_level=1.0, noise_level_bounds=(1e-05, 100000.0)),
            alpha=1e-10,
...
def fit_wrapgp(X, Y, XX):
  # define circular manifold
  circle = Hypersphere(dim = 1)
  metric = circle.metric
  
  gr = GeodesicRegression(circle, center_X = False, method = "extrinsic")
  gr.fit(np.squeeze(X), Y)
  intercept_hat, coef_hat = gr.intercept_, gr.coef_
  
  # define prior
  def prior(X):
    #return circle.random_point(n_samples = X.shape[0])
    return(metric.exp(X * coef_hat, base_point = intercept_hat))
  
  # RBF kernel
  #kernel = ConstantKernel(1.0, (1e-3, 1e3)) * RBF(10, (1e-2, 1e2))
  
  # create Wrapped Gaussian Process
  wgp = WrappedGaussianProcess(space = circle, prior = prior)

  # fit model to training data
  wgp.fit(X, Y)

  # predict the mean and standard deviation on the circle
  mu, sd = wgp.predict(XX, return_tangent_std = True)

  #XX = np.sort(XX)
  #samps = wgp.sample_y(XX, 1000)
  #for j in range(10):
  #  mu = samps[:,:,j]
  #  mu2 = np.arccos(mu[:,0])
  #  for i in range(mu2.shape[0]):
  #    if np.arcsin(mu[i, 1]) < 0:
  #      mu2[i] = 2*np.pi - mu2[i]
  #  plt.plot(XX, mu2, "-")
  #plt.show()
  ...
  draws = wgp.sample_y(XX, 500)
  draws2 = np.zeros((500, len(XX)))
  for i in range(500):
    draw = draws[:,:,i]
    draw2 = np.arccos(draw[:,0])
    for j in range(draw.shape[0]):
      if np.arcsin(draw[j,1]) < 0:
        draw2[j] = 2*np.pi - draw2[j]
    draws2[i,:] = draw2
   
  return(mu2, sd[:,0], draws2)
