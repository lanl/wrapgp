...
if(method == 1){  # our wrapped gp
  ...
  #settings$beta$mu0[2] <- slope_est(X, Y, 15)
  #fit2 <- fit_wrapped_gp(X, Y, g = 1e-4, kmin = -2, theta = NULL, tau2 = NULL, beta = NULL, sigma2 = NULL,
  #                      nmcmc = nmcmc, temp = F, settings = settings)
  #settings$beta$mu0[2] <- slope_est(X, Y, 20)
  #fit3 <- fit_wrapped_gp(X, Y, g = 1e-4, kmin = -2, theta = NULL, tau2 = NULL, beta = NULL, sigma2 = NULL,
  #                      nmcmc = nmcmc, temp = F, settings = settings)
  #fits <- list(fit1, fit2, fit3)
  #max_ll <- unlist(lapply(fits, function(fit) max(fit@ll_y)))
  #fit <- fits[[which.max(max_ll)]]
  
  name <- "wgp_ours"
}

if(method == 2){ #jona-lasinio's wrapped gp
  beta_init <- slope_est(X, Y, 15)
  settings <- mcmc_settings(X, theta = list(a = 2, b = 1, u = 1.5, prior = "gamma"),
  ...
  #settings$beta$mu0[2] <- slope_est(X, Y, 15)
  #fit2 <- fit_wrapped_gp(X, Y, g = NULL, theta = NULL, tau2 = NULL, beta = NULL, sigma2 = NULL,
  #                       nmcmc = nmcmc, temp = F, method = "jonas", k_min = -5, k_max = 5, settings = settings)
  #settings$beta$mu0[2] <- slope_est(X, Y, 20)
  #fit3 <- fit_wrapped_gp(X, Y, g = NULL, theta = NULL, tau2 = NULL, beta = NULL, sigma2 = NULL,
  #                       nmcmc = nmcmc, temp = F, method = "jonas", k_min = -5, k_max = 5, settings = settings)
  #fits <- list(fit1, fit2, fit3)
  #max_ll <- unlist(lapply(fits, function(fit) max(fit@ll_y)))
  #fit <- fits[[which.max(max_ll)]]
